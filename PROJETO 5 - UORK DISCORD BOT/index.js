...
const apiKey = "SUA-KEY-DA-UORK"; // API KEY DA UORK
...
client.once('ready', async () => {
    ...
    const ServerID = 'ID-DO-SEU-SERVIDOR';
    ...
    if (interaction.commandName === 'puxar-info') {
        ...
        try {
          ...
          if (responseData.status && responseData.status === '404') {
              await interaction.reply(':red_x_cross: | E-mail não encontrado.');
            } else {
            ...
            await interaction.reply(':red_x_cross: | Ocorreu um erro ao processar a requisição.');
          }
        }
      });
